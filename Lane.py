...
LANEWIDTH = 0.44  # highway lane width in US: 3.7 meters
input_scale = 4
output_frame_scale = 4
N = 4 # buffer previous N lines

# fullsize:1280x720
x = [frame_width * 194 / 1280, frame_width * 1117 / 1280, frame_width * 705 / 1280, frame_width * 575 / 1280]
y = [frame_height * 719 / 720, frame_height * 719 / 720, frame_height * 461 / 720, frame_height * 461 / 720]
X = [frame_width * 290 / 1280, frame_width * 990 / 1280, frame_width * 990 / 1280, frame_width * 290 / 1280]
Y = [frame_height * 719 / 720, frame_height * 719 / 720, frame_height * 0 / 720, frame_height * 0 / 720]
...
X_b = [frame_width * 574 / 1280, frame_width * 706 / 1280, frame_width * 706 / 1280, frame_width * 574 / 1280]
Y_b = [frame_height * 719 / 720, frame_height * 719 / 720, frame_height * 0 / 720, frame_height * 0 / 720]
...
def create_output_frame(offcenter, pts, undist_ori, fps, curvature, curve_direction, binary_sub, threshold=0.6):
    ...
    whole_frame = np.zeros((int(355),int(w*2.34)+20, 3), dtype=np.uint8)
...
def tracker(binary_sub, ploty):

    left_fit, right_fit = window_search(left_lane.prev_poly, right_lane.prev_poly, binary_sub, margin=100/input_scale)
...
def process_frame(img):
    ...
    img_undist = cv2.resize(img, (0,0), fx=1/input_scale, fy=1/input_scale)
    ...
    if left_lane.detected:  # start tracker
        tracker(binary_sub, ploty)
    else:  # start detector
        detector(binary_sub, ploty)
    ...
    output = create_output_frame(offcenter, pts, img, fps, curvature, curve_direction, binary_sub)
    return output

def process_frame_get_angle(img, current_angle):
   ...
   new_angle = 0.0
   if (offcenter > 0):
       new_angle = (current_angle + offcenter * 50) / 2
   else:
       new_angle = (current_angle - offcenter * 50) / 2
